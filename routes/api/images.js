...
router.get('/', function(req, res, next) {
    return ImageCtrl.getImagesByUsername(0, "Demo", function(err, images){
        if(err) {
            console.log(err);
            return res.send(err);
        }
...
router.post('/emotiondetect/:imageid', function(req, res, next) {
    try {
        return TestUtils.detectImageByIdEmotion(req.params.imageid, function (err, emotionObj, mainEmotion) {
            ...
            if (err) {
                var emotionNew = [{"emotion": "Not detected", "scores" : "Not detected" }];
                return res.status(200).jsonp(emotionNew);
            }

...
/*TestUtils.prototype.populateDBwithMockData = function (numberDocs) {
 ...
 */
TestUtils.prototype.generateMultipleDBdocs = function(number, save, callback) {
    number = number || 0;
    var resultArray = [];
        ...
        if(save) {
            this.imageCtrl.addMockImage(resultArray[i], function(error, store){
                if(error) {
                    ...
                    console.log("Image successfully saved in database:\n" + JSON.stringify(store));
                }
            });
        }

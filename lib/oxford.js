...
Oxford.prototype.extractMainEmotion = function(responseObj){
    if(responseObj == undefined) return responseObj;
 ...
 * @returns {undefined}
 */
Oxford.prototype._parseResponse = function (error, response, callback) {
    var emotion;
    var err;
    try {
        if (response == undefined) throw new Error("Response is empty").stack;
...
Oxford.prototype._getEmotion = function (image, callback) {
        ...
        return request({
            ...
            headers: {
                ...
                try {
                    if (error) throw new Error("Couldn't reach Oxford Service server").stack;

                } catch (e) {
                    console.log(e);
                }
                self._parseResponse(error, response, callback);
            });
};
